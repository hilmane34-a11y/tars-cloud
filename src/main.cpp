...
inline void dacMute(){dacWrite(AUDIO_DAC_PIN,128);}
...
class DACOut:public AudioStream{
  ...
  void run(){
    ...
    while(active){
      ...
      // +10% dari level PCM sebelumnya (75% -> 82.5%)
      int32_t v=((int32_t)sample*825)/1000;

      // soft fade-in untuk mengurangi pop/noise awal
      if(!started){
        dacCenter();
        next=micros();
        frac=0;
      ...
      // TPDF dither ringan sebelum kuantisasi 8-bit
      int32_t d=(int32_t)(rnd()>>24)-(int32_t)(rnd()>>24);
      int32_t out=((v+d)+32768+128)>>8;
    ...
    // kembali ke midpoint DAC, bukan 0
    dacMute();
    active=false;
    task=nullptr;
    vTaskDelete(nullptr);
    ...
    while(done<frames){
      ...
      for(size_t i=0;i<chunk;i++){
        ...
        if(info.channels==1)
          ...
          s=(int16_t)(
            ((int32_t)(
              (int16_t)(d[k*4]|((uint16_t)d[k*4+1]<<8))
            )+
...
void setup(){
  ...
  if(WiFi.status()==WL_CONNECTED){
    ...
    if(syncTime())
      ...
      Serial.println("TARS: WIFI READY, NTP FAILED");
  }else{
