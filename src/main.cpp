...
void makeMFCCFrame(const int16_t*src,float*out){
  ...
  for(int c=0;c<MFCC;c++){
    float v=0;
    for(int m=0;m<MEL;m++)v+=melEnergy[m]*cosf(PI/MEL*(m+.5f)*c);
    out[c]=v;
  }
}
...
void normalizeFeatures(){
  for(int c=0;c<MFCC;c++){
    ...
    for(int f=0;f<KWS_FRAMES;f++)
      kwsFeat[f][c]=constrain((kwsFeat[f][c]-mean)/sd,-3.0f,3.0f)/3.0f*127.0f;
  }
}

bool recordKWS(){
  if(!micOK)return false;
  int32_t raw[IO_BUF/4];int16_t pcm[IO_BUF/4];
  static int16_t pre[RATE*PREROLL_MS/1000];
  const size_t PN=sizeof(pre)/2;
  size_t pp=0,pc=0,frames=0;
  bool voice=false;byte trigger=0;uint32_t start=millis();
  ...
  while(frames<KWS_FRAMES)
    for(int c=0;c<MFCC;c++)kwsFeat[frames][c]=kwsFeat[frames-1][c],frames++;
  normalizeFeatures();
  return true;
}
...
float distanceTo(const int8_t*t){
  ...
  for(int i=0;i<KWS_FRAMES*MFCC;i++){
    int8_t tv=(int8_t)pgm_read_byte(&t[i]);
    s+=fabsf(kwsFeat[i/MFCC][i%MFCC]-(float)tv);
  }
...
bool downloadMP3(const String&url,const String&text){
  ...
  uint8_t z[IO_BUF];int len=h.getSize();size_t total=0;
...
String localTimeText(){
  ...
  char s[150];
  snprintf(s,sizeof(s),"Pukul %02d lewat %02d menit WIB. %s, %02d %s tahun %04d.",t.tm_hour,t.tm_min,d[t.tm_wday],t.tm_mday,m[t.tm_mon],t.tm_year+1900);
  return String(s);
}
...
void answer(const String&q){
  ...
  if(l.indexOf("pengalaman")>=0){
    singMode=false;oledText="Aku adalah TARS, AI yang dikembangkan oleh Ilman.";
    oledPos=oledPage=0;oledBase("STANDBY",oledText);return;
  }
