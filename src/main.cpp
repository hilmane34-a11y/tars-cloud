...
const size_t PCM_PREBUFFER=4096;
...
class AudioRingStream:public Stream{
  ...
  void rx(){
    ...
    while(!stopFlag){
      ...
      if(av>0){
        size_t f=AUDIO_RING_SIZE-available();
  ...
  size_t push(const uint8_t*p,size_t x){
    ...
    size_t f=AUDIO_RING_SIZE-n;x=min(x,f);
    size_t first=min(x,AUDIO_RING_SIZE-h);
    memcpy(b+h,p,first);if(x>first)memcpy(b,p+first,x-first);
...
public:
  void start(WiFiClient&s,int len=-1){
    stop();portENTER_CRITICAL(&mux);
    h=t=n=0;done=false;stopFlag=false;running=true;portEXIT_CRITICAL(&mux);
  ...
  int available()override{
    portENTER_CRITICAL(&mux);int r=n;portEXIT_CRITICAL(&mux);return r;
  }
  ...
  int read(uint8_t*p,size_t x){
    ...
    if(take){
      ...
      memcpy(p,b+t,first);if(take>first)memcpy(p,b,take-first);
  ...
  int peek()override{
    portENTER_CRITICAL(&mux);int r=n?b[t]:-1;portEXIT_CRITICAL(&mux);return r;
  }
...
public:
  ...
  size_t write(const uint8_t*p,size_t n)override{
    if(!p||!n)return 0;if(!t0)t0=micros();decBytes+=n;
    size_t done=0;while(done<n){size_t w=out->write(p+done,n-done);if(w)done+=w;else{delay(1);yield();}}
    dacBytes+=done;return done;
  }
...
EncodedAudioStream dec(&pcmProbe,&codec);
...
bool initDAC(){
  ...
  dec.addNotifyAudioChange(pcmProbe);wavDec.addNotifyAudioChange(pcmProbe);
  Serial.println("TARS: DAC GPIO26 READY");return true;
}
...
bool bootWiFi(){
  Serial.println("TARS: WIFI CONNECTING...");uint32_t st=millis();
...
bool syncTime(){
  ...
  for(int a=1;a<=4;a++){
    ...
    for(int i=0;i<20;i++){time_t now=time(nullptr);if(now>=1704067200){struct tm t;localtime_r(&now,&t);Serial.printf("TARS: NTP VALID %04d-%02d-%02d %02d:%02d:%02d\n",t.tm_year+1900,t.tm_mon+1,t.tm_mday,t.tm_hour,t.tm_min,t.tm_sec);ntpOK=true;return true;}delay(500);}
  }
...
void sttEvent(WStype_t type,uint8_t*payload,size_t length){
  ...
  if(type==WStype_DISCONNECTED){sttConnected=false;if(!sttDone)sttError=true;Serial.println("TARS: STT WS DISCONNECTED");return;}
  ...
  if(t=="ready"){sttReady=true;Serial.println("TARS: STT REALTIME READY");}
  ...
  else if(t=="error"){sttError=true;sttDone=true;Serial.print("TARS: STT ERROR = ");Serial.println(j["error"].as<String>());}
}
...
bool startSTT(){
  ...
  uint32_t st=millis();
  while(!sttReady&&!sttError&&millis()-st<7000){sttWS.loop();delay(2);yield();}
...
String ask(const String&q){
  ...
  if(deserializeJson(x,r))return "";String s=x["response"].as<String>();s.trim();return s;
}
...
bool streamAudio(const String&url,const String&text){
  ...
  if(!h.begin(c,url))return false;h.setTimeout(60000);h.addHeader("Content-Type","application/json");
  const char*keys[]={"Content-Type","X-TARS-TTS","X-TARS-TTS-FORMAT","X-TARS-TTS-RATE","X-TARS-TTS-CHANNELS","X-TARS-TTS-BITS"};
  h.collectHeaders(keys,6);
  ...
  String ct=h.header("Content-Type"),fmt=h.header("X-TARS-TTS-FORMAT"),rate=h.header("X-TARS-TTS-RATE");
  String ch=h.header("X-TARS-TTS-CHANNELS"),bits=h.header("X-TARS-TTS-BITS"),engine=h.header("X-TARS-TTS");ct.toLowerCase();
  Serial.printf("TARS: TTS=%s FORMAT=%s RATE=%s\n",engine.c_str(),fmt.c_str(),rate.c_str());

  WiFiClient*stream=h.getStreamPtr();if(!stream){h.end();return false;}
  if(!dacOK)dacOK=initDAC();if(!dacOK){h.end();return false;}

  int contentLen=h.getSize();audioRing.start(*stream,contentLen);playing=true;

  /* PCM STREAM */
  if(fmt.equalsIgnoreCase("PCM")){
    uint32_t sr=rate.toInt();uint8_t channels=ch.toInt(),bp=bits.toInt();
    if(!sr)sr=24000;if(!channels)channels=1;if(!bp)bp=16;
    Serial.printf("TARS: PCM STREAM %lu Hz / %d ch / %d bit\n",(unsigned long)sr,channels,bp);

    AudioInfo ai;ai.sample_rate=sr;ai.channels=channels;ai.bits_per_sample=bp;
    pcmProbe.reset();pcmProbe.setAudioInfo(ai);

    size_t target=PCM_PREBUFFER;
    uint32_t ps=millis();
    while(audioRing.available()<target&&!audioRing.finished()){
      if(millis()-ps>8000){Serial.println("TARS: PCM PREBUFFER TIMEOUT");audioRing.stop();h.end();playing=false;return false;}
      delay(1);yield();
    }

    if(!audioRing.available()){audioRing.stop();h.end();playing=false;return false;}
    oledStartSpeak(text);

    uint8_t buf[1024];bool started=false;uint32_t start=millis();
    while(true){
      int n=audioRing.read(buf,sizeof(buf));
      if(n){
        if(n&1){uint8_t x;audioRing.read(&x,1);}
        int usable=n&~1;
        if(usable){pcmProbe.write(buf,usable);started=true;}
      }else{
    ...
    Serial.printf("TARS: PCM STREAM=%lu ms\n",(unsigned long)(millis()-start));
  ...
  if(!isWav){
    ...
    while(audioRing.available()<(int)target&&!audioRing.finished()){
      if(millis()-ps>10000){Serial.println("TARS: MP3 PREBUFFER TIMEOUT");audioRing.stop();h.end();playing=false;return false;}
      delay(1);yield();
    }
    int pre=audioRing.available();
    if(!pre){audioRing.stop();h.end();playing=false;Serial.println("TARS: MP3 EMPTY");return false;}
    Serial.printf("TARS: MP3 PREBUFFER=%d/%u BYTES\n",pre,(unsigned)target);
    pcmProbe.reset();dec.begin();copier.begin(dec,audioRing);
    ...
    while(true){
      ...
      if(copied&&!started){started=true;AudioInfo ai=codec.audioInfo();Serial.printf("TARS: MP3 %lu Hz / %d ch / %d bit\n",(unsigned long)ai.sample_rate,ai.channels,ai.bits_per_sample);oledStartSpeak(text);}
      if(audioRing.finished()&&audioRing.available()==0)break;
      if(before==after)yield();
    }
    audioRing.stop();dec.end();pcmProbe.report();h.end();playing=false;
  ...
  /* WAV */
  Serial.println("TARS: WAV AUTO FORMAT");oledStartSpeak(text);
  pcmProbe.reset();wavDec.begin();copier.begin(wavDec,audioRing);
  bool started=false;uint32_t start=millis();
  while(true){
    int before=audioRing.available();bool copied=copier.copy();int after=audioRing.available();
...
void runAlarm(){
  ...
  alarmLastDay=t.tm_yday;alarmRunning=true;Serial.println("TARS: ALARM 06:00 WIB");oledSetStatus("ALARM");
  uint32_t start=millis();
  while(millis()-start<ALARM_DURATION_MS){
    if(!wifiOK())break;
    uint32_t before=millis();streamAudio(String(TARS_CLOUD_URL)+"/tts",ALARM_TEXT);
    while(millis()-before<3000&&millis()-start<ALARM_DURATION_MS){delay(20);yield();}
  }
...
void setup(){
  ...
  Serial.println("TARS: DAC GPIO26 INTERNAL DAC");Serial.println("TARS: MP3/WAV AUTO FORMAT");Serial.println("TARS: INMP441 RIGHT GPIO34");
  ...
  Serial.printf("TARS: STREAM EOF IDLE=%lu ms\n",(unsigned long)STREAM_EOF_IDLE_MS);Serial.println("TARS: ALARM=06:00 WIB");
...
void loop(){
  ...
  if(q.length())processQuestion(q);else oledSetListening();
  delay(1);
}
