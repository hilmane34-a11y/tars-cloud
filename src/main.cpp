...
public:
  ...
  int read(uint8_t*p,size_t x){
    ...
    if(take){
      ...
      memcpy(p,b+t,first);
      if(take>first)memcpy(p,b+first,take-first);
...
class PCMProbeStream:public AudioStream{
  ...
  uint64_t decBytes=0,dacBytes=0;uint32_t t0=0;
public:
  ...
  size_t write(const uint8_t*p,size_t n)override{
    ...
    size_t done=0;
    while(done<n){
      size_t w=out->write(p+done,n-done);
      if(w)done+=w;else{delay(1);yield();}
    }
...
bool initDAC(){
  ...
  wavDec.addNotifyAudioChange(pcmProbe);
  Serial.println("TARS: DAC GPIO26 RIGHT READY");
  Serial.println("TARS: MP3 44.1k -> RESAMPLE 22.05k/16bit -> GPIO26");
  Serial.println("TARS: WAV -> PCM -> GPIO26");
  return true;
}
...
bool bootWiFi(){
  ...
  if(WiFi.status()!=WL_CONNECTED){
    ...
    oledSetStatus("WIFI FAILED");
    return false;
  }
  String ip=WiFi.localIP().toString();
  Serial.print("TARS: WIFI CONNECTED IP=");
  Serial.println(ip);
  oledSetStatus("WIFI CONNECTED");
  delay(1200);
  return true;
}

/* NTP: hanya status sinkronisasi boot sekarang yang dianggap valid. */
...
bool syncTime(){
  ...
  for(int a=1;a<=4;a++){
    ...
    while(millis()-st<10000){
      if(ntpSyncEvent||sntp_get_sync_status()==SNTP_SYNC_STATUS_COMPLETED){
        ...
        if(now>=1704067200){
          ...
          oledText=d;oledTypePos=d.length();
          delay(2500);
          return true;
        }
      }
    ...
    if(a<4){
      ...
      ntpSyncEvent=false;
      if(!sntp_restart())Serial.println("TARS: NTP RESTART FAILED");
      delay(1000);
    }
  }
...
void sttEvent(WStype_t type,uint8_t*payload,size_t length){
  ...
  if(type==WStype_DISCONNECTED){
    ...
    oledSetStatus("STT DISCONNECTED");
    return;
  }
...
String stopSTT(uint32_t samples){
  ...
  sttWS.sendTXT(msg);Serial.println("TARS: STT END SENT");
...
bool streamAudio(const String&url,const String&text){
  ...
  if(!isWav){
    ...
    Serial.println("TARS: MP3 RESAMPLE TARGET=22050 Hz / 16 bit");
    ...
    if(!pre){
      ...
      Serial.println("TARS: MP3 EMPTY");return false;
    }

    Serial.printf("TARS: MP3 PREBUFFER=%d/%u BYTES\n",pre,(unsigned)target);

    pcmProbe.reset();dec.begin();
    ...
    Serial.println("TARS: MP3 TARGET 22050 Hz / 16 bit");

    copier.begin(dec,audioRing);

    bool started=false;uint32_t start=millis();
    while(true){
      int before=audioRing.available();bool copied=copier.copy();
    ...
    mp3Resample.flush();
    audioRing.stop();dec.end();mp3Resample.end();
    pcmProbe.report();h.end();playing=false;
...
void runAlarm(){
  ...
  while(millis()-start<ALARM_DURATION_MS){
    ...
    streamAudio(String(TARS_CLOUD_URL)+"/tts",ALARM_TEXT);

    while(millis()-before<3000&&millis()-start<ALARM_DURATION_MS){
      delay(20);yield();
    }
  }

  playing=false;alarmRunning=false;oledSetListening();
...
void setup(){
  ...
  Serial.println("TARS: MP3 44.1k -> 22.05k/16bit");
  Serial.println("TARS: WAV AUTO FORMAT");
