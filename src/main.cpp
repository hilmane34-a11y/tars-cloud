...
/* INMP441: SCK=18 WS=19 SD=34, RIGHT */
...
bool recordSTT(){
 ...
 bool voice=false;byte trigger=0;
 uint32_t wait=millis(),vs=0,lastVoice=0;

 oledBase("LISTENING","SPEAK NOW");
 Serial.println("TARS: LISTENING");

 while((!voice&&millis()-wait<LISTEN_MAX)||(voice&&millis()-vs<REC_MAX)){
  size_t n;int32_t peak;
  if(!readMic(pcm,IO_BUF/4,n,peak))continue;

  if(!voice){
   for(size_t i=0;i<n;i++){
    pre[pp]=pcm[i];pp=(pp+1)%PN;if(pc<PN)pc++;
   }
   if(peak>=MIC_TH){
    if(++trigger>=2){
     voice=true;vs=lastVoice=millis();
     size_t st=pc==PN?pp:0;
     for(size_t i=0;i<pc;i++){
      int16_t s=pre[(st+i)%PN];f.write((uint8_t*)&s,2);samples++;
     }
    }
...
bool embeddedMP3(){
 LittleFS.remove(PLAY_FILE);
 File f=LittleFS.open(PLAY_FILE,FILE_WRITE);if(!f)return false;
