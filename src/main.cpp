...
public:
  ...
  void flush()override{
    portENTER_CRITICAL(&mux);h=t=n=0;portEXIT_CRITICAL(&mux);
  }
...
class PCMProbeStream:public AudioStream{
  AudioStream*out;AudioInfo info;
  ...
  for(;;){
    ...
    if(now-oledLastWave>=OLED_WAVE_MS){
      ...
      if(oledText.length()){
        ...
        uint32_t lineNo=0,target=oledPage*4;uint8_t shown=0;
        String line;bool next=false;

        for(size_t i=0;i<=s.length();i++){
          char c=i<s.length()?s[i]:'\0';
          if(c=='\n'||c=='\0'){
            if(lineNo>=target&&shown<4)
              oled.setCursor(3,29+shown++*8),oled.print(line);
            line="";lineNo++;
...
bool initDAC(){
  ...
  Serial.println("TARS: AUDIO OUTPUT 22050 Hz / 16 bit");
  Serial.println("TARS: MP3 -> 22050 Hz -> GPIO26");
  Serial.println("TARS: WAV -> 22050 Hz -> GPIO26");
  return true;
}
...
bool bootWiFi(){
  ...
  if(WiFi.status()!=WL_CONNECTED){
    ...
    oledSetStatus("WIFI FAILED");return false;
  }

  Serial.print("TARS: WIFI CONNECTED IP=");
  Serial.println(WiFi.localIP());
  oledSetStatus("WIFI CONNECTED");delay(1200);
  return true;
}
...
bool syncTime(){
  ...
  for(int a=1;a<=4;a++){
    ...
    while(millis()-st<10000){
      if(ntpSyncEvent||sntp_get_sync_status()==SNTP_SYNC_STATUS_COMPLETED){
        ...
        if(now>=1704067200){
          ...
          ntpOK=true;
          String d=String(t.tm_mday<10?"0":"")+String(t.tm_mday)+"/"+
            String(t.tm_mon+1<10?"0":"")+String(t.tm_mon+1)+"/"+
            String(t.tm_year+1900)+" "+
            String(t.tm_hour<10?"0":"")+String(t.tm_hour)+":"+
            String(t.tm_min<10?"0":"")+String(t.tm_min)+":"+
            String(t.tm_sec<10?"0":"")+String(t.tm_sec);

          oledSetStatus("NTP OK");oledText=d;
          oledTypePos=d.length();delay(2500);
          return true;
        }
      }
      delay(100);yield();
    ...
    if(a<4){
      Serial.println("TARS: NTP RETRY");
      oledSetStatus("NTP RETRY");
      ntpSyncEvent=false;sntp_restart();delay(1000);
    }
  }
...
void sttEvent(WStype_t type,uint8_t*payload,size_t length){
  ...
  if(type==WStype_DISCONNECTED){
    ...
    oledSetStatus("STT DISCONNECTED");return;
  }

  if(type==WStype_ERROR){
    sttError=true;Serial.println("TARS: STT WS ERROR");
...
bool startSTT(){
  ...
  sttWS.disconnect();sttWS.onEvent(sttEvent);
...
bool streamAudio(const String&url,const String&text){
  ...
  Serial.print("TARS: TTS CONTENT-TYPE=");Serial.println(ct);
  Serial.print("TARS: TTS STATUS=");Serial.println(engine);
  Serial.print("TARS: TTS FORMAT=");Serial.println(fmt);
  ...
  while(audioRing.available()<(int)target&&!audioRing.finished()){
    if(millis()-ps>10000){
      Serial.println("TARS: AUDIO PREBUFFER TIMEOUT");
  ...
  if(!audioRing.available()){
    Serial.println("TARS: AUDIO EMPTY");
    audioRing.stop();h.end();playing=false;return false;
  }
  ...
  if(!isWav){
    ...
    Serial.println("TARS: MP3 -> 22050 Hz / 16 bit");

    dec.begin();
    AudioInfo src=codec.audioInfo();

    if(!mp3Resample.begin(src,22050)){
      Serial.println("TARS: MP3 RESAMPLER ERROR");
      dec.end();audioRing.stop();h.end();playing=false;
      return false;
    }
    ...
    Serial.println("TARS: WAV -> RESAMPLE 22050 Hz / 16 bit");

    wavDec.begin();
    AudioInfo src=wav.audioInfo();

    Serial.printf("TARS: WAV SOURCE %lu Hz / %d ch / %d bit\n",
      (unsigned long)src.sample_rate,src.channels,src.bits_per_sample);

    if(!wavResample.begin(src,22050)){
      Serial.println("TARS: WAV RESAMPLER ERROR");
      wavDec.end();audioRing.stop();h.end();playing=false;
      return false;
    }
...
String systemStatus(){
  ...
  s+="RAM bebas "+String(ESP.getFreeHeap()/1024.0,1)+" KB, minimum "+
    String(ESP.getMinFreeHeap()/1024.0,1)+" KB, blok terbesar "+
    String(ESP.getMaxAllocHeap()/1024.0,1)+" KB.\n";

  s+="Flash "+String(ESP.getFlashChipSize()/1024.0/1024.0,1)+" MB, sketch "+
    String(ESP.getSketchSize()/1024.0,1)+" KB, ruang sketch bebas "+
    String(ESP.getFreeSketchSpace()/1024.0,1)+" KB.\n";

  if(LittleFS.begin(true))
    s+="LittleFS total "+String(LittleFS.totalBytes()/1024.0,1)+" KB, terpakai "+
...
bool playLocalAlarm(){
  if(!dacOK)dacOK=initDAC();
  if(!dacOK)return false;

  Serial.printf("TARS: OFFLINE ALARM PLAY %u BYTES\n",
    (unsigned)(alarm_end-alarm_start));

  oledSetStatus("ALARM");playing=true;
  alarmStream.begin();pcmProbe.reset();
  ...
  playing=false;oledSetListening();
...
void processQuestion(const String&q){
  ...
  oledShowText(q,"STT");
  Serial.println("TARS: STT FINAL DISPLAY");
  delay(800);
  ...
  Serial.println("TARS: ASK RESULT DISPLAY");
  delay(800);
