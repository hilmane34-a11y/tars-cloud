...
uint32_t dacCalls=0,dacSamples=0;
int32_t dacPeak=0;
...
void oledHeader(const char *s){
  ...
  oled.drawLine(3,9,124,9,SSD1306_WHITE);
...
void dacTask(void*){
  ...
  while(dacRunning){
    if(!dacAvailable()){
      ...
      last=micros(); taskYIELD(); continue;
    }
    ...
    uint32_t now=micros(),e=now-last;
    if(e<45)delayMicroseconds(45-e);
    last=micros();

    frac+=1000000UL;
    uint32_t us=frac/PLAY_RATE;
    frac%=PLAY_RATE;
    if(us>0)delayMicroseconds(us-1);

    int32_t p=s<0?-s:s;
    if(p>dacPeak)dacPeak=p;

    dacWrite(AUDIO_DAC_PIN,pcmToDAC(s));
    dacCalls++; dacSamples++;
    if(!(dacSamples&1023))taskYIELD();
  }
  dacWrite(AUDIO_DAC_PIN,0);
  dacTaskHandle=nullptr;
  vTaskDelete(nullptr);
}

bool initDAC(){
  dacWrite(AUDIO_DAC_PIN,0);
  dacMutex=xSemaphoreCreateMutex();
...
bool startDAC(){
  ...
  xSemaphoreGive(dacMutex);
  dacRunning=true; dacWrite(AUDIO_DAC_PIN,0);
...
bool recordMic(){
  ...
  File f=LittleFS.open(STT_FILE,"w");
  if(!f)return false;
  ...
  while(millis()-start<RECORD_MAX_MS){
    ...
    for(size_t i=0;i<n;i++){
      int32_t v=pcm[i];if(v<0)v=-v;if(v>peak)peak=v;
    }
...
bool wifiOK(){
  if(!wifiManagerBegin())return false;
  if(!wifiManagerConnect(true))return false;
  Serial.printf("TARS: WIFI READY %s\n",WiFi.localIP().toString().c_str());
  return syncTime();
}

String stt(){
  File f=LittleFS.open(STT_FILE,"r");
  if(!f)return "";
...
bool downloadMP3(const String &text){
  ...
  File f=LittleFS.open(MP3_FILE,"w");
  if(!f){h.end();return false;}

  WiFiClient *s=h.getStreamPtr();
  uint8_t b[1024];int left=h.getSize();

  while(h.connected()){
    size_t a=s->available();
    if(a){
      size_t n=s->readBytes(b,min(a,sizeof(b)));
      if(n)f.write(b,n);
...
public:
  size_t write(const uint8_t *b,size_t n)override{
    if(!b||!n)return 0;
    size_t s=n/2;
    return writeDAC((const int16_t*)b,s)*2;
  }
  size_t write(uint8_t v)override{return write(&v,1);}
};

bool playMP3(){
  File file=LittleFS.open(MP3_FILE,"r");
  if(!file)return false;

  Serial.printf("TARS: PLAY MP3 SIZE=%u\n",(unsigned)file.size());

  FileStream input(file);
  MP3DecoderHelix decoder;
  DACStream output;

  AudioInfo info;
  info.sample_rate=PLAY_RATE;
  info.channels=1;
  ...
  StreamCopy copier(output,decoder);
...
void processQuestion(){
  ...
  Serial.println("TARS: TTS");

  if(!downloadMP3(a)){
    Serial.println("TARS: TTS DOWNLOAD FAILED");
    oledHeader("READY");
    return;
  }
...
void setup(){
  ...
  if(!LittleFS.begin(true))
    ...
    Serial.println("TARS: LITTLEFS READY");

  if(!initMic())
    Serial.println("TARS: MIC ERROR");
  ...
  if(!wifiOK()){
    Serial.println("TARS: WIFI/NTP FAILED");
