...
bool recordSTT(){
    ...
    if(!voice){
      ...
      if(peak>=MIC_TH){
        if(++trigger>=2){
          ...
          // pcm TIDAK ditulis lagi:
          // blok ini sudah termasuk di prebuffer.
        }
      }else trigger=0;
    }else{
  ...
  p32(h+16,16);p16(h+20,1);p16(h+22,1);
  p32(h+24,RATE);p32(h+28,RATE*2);
  p16(h+32,2);p16(h+34,16);
...
String stt(){
  ...
  JsonDocument j;
  DeserializationError e=deserializeJson(j,body);
  if(e){
    Serial.printf("STT JSON ERROR: %s\r\n",e.c_str());
    return "";
  }
...
class DACOut:public AudioStream{
  ...
  size_t count(){
    return head>=tail?head-tail:DAC_BUF-tail+head;
  }
