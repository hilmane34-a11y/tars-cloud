...
Serial.println("TARS: NTP START");configTime(7*3600,0,"pool.ntp.org","time.google.com","time.cloudflare.com");
for(int a=1;a<=4;a++){
...
for(size_t i=0;i<preCount;i++){sendBuf[nsend++]=preBuf[(start+i)%PREROLL_SAMPLES];
...
dec.end();pcmProbe.report();playing=false;if(!keepSpecial)oledSetListening();return ok;
}
...
ct.toLowerCase();Serial.println("TARS: TTS CONTENT-TYPE="+ct);
Serial.println("TARS: TTS STATUS="+engine);Serial.println("TARS: TTS FORMAT="+fmt);
...
wifiManagerBegin();if(bootWiFi())syncTime();
oledSetListening();
}
