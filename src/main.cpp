...
bool recordSTT(){
  LittleFS.remove(STT_FILE);File f=LittleFS.open(STT_FILE,FILE_WRITE);
  if(!f)return false;
...
String stt(){
  ...
  JsonDocument j;
  if(deserializeJson(j,body.substring(a,b+1))){
    Serial.println("STT JSON ERROR");
    return "";
  }

  JsonVariant v=j["text"];
  if(v.isNull())return "";
