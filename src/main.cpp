// Full main.cpp (revised) for hilmane34-a11y/tars-cloud
// Includes safe A2DP callback handling and safe deregistration before delete.
...
void oledDrawMechanical(bool speaking) {
    ...
    if (!speaking) {
        ...
        for (int i = 0; i < 24; i++) {
            int x = 3 + (i * 5);
            if (x > 123) break;
            uint8_t index = (i + oledMechanicalFrame) % 24;
            int height = normalPattern[index];
            oled.drawLine(x, baseY - height, x + 3, baseY - height, SSD1306_WHITE);
            if (i < 23) {
                int nextX = x + 5;
                if (nextX <= 125) {
                    oled.drawLine(x + 3, baseY - height, nextX, baseY - normalPattern[(index + 1) % 24], SSD1306_WHITE);
                }
            }
        }
        int pulseX = 45 + (oledMechanicalFrame % 17);
        oled.drawLine(pulseX, 57, pulseX + 3, 57, SSD1306_WHITE);
        return;
    }
    ...
    for (int i = 0; i < 6; i++) {
        ...
        if (i == (oledMechanicalFrame % 6)) {
            height = 8;
        } else if (i == ((oledMechanicalFrame + 5) % 6) || i == ((oledMechanicalFrame + 1) % 6)) {
            height = 5;
        }
        oled.drawLine(x, baseY - height, x, baseY, SSD1306_WHITE);
        oled.drawLine(x - 3, baseY - height, x, baseY, SSD1306_WHITE);
        oled.drawLine(x, baseY, x + 3, baseY - height, SSD1306_WHITE);
    }
...
// OLED TYPING START
// ============================================================
void oledStartTyping(const String &text) {
    if (!oledReadyFlag) return;
    oledAnswer = text;
    oledTypedChars = 0;
    oledLastType = millis();
...
void oledDrawTypedText(bool speaking) {
    ...
    for (size_t i = 0; i < visible.length(); i++) {
        ...
        if (c == '\n') {
            ...
            oled.setCursor(startX, startY + line * 9);
            continue;
        }
        if (column >= maxChars) {
            line++;
...
// NETWORK
// ============================================================
static const char *WORKER_URL = "https://tars-cloud-v1.hilmane34.workers.dev";
...
// ============================================================
// NETWORK / TTS / NTP / WiFi (unchanged from original)
// ============================================================
...
bool syncNTP() {
    Serial.println("TARS: TIME INVALID - NTP REQUIRED");
...
// ============================================================
// ASK AI and downloadTTS (unchanged from original)
// ============================================================
String askAI(const String &question) {
    if (!connectWiFi(true)) return "";
    if (!ensureTimeValid()) return "";
    oledShowProcessing();
    ...
    if (httpCode < 200 || httpCode >= 300) { http.end(); return ""; }
...
bool downloadTTS(const String &text) {
    if (!connectWiFi(true)) return false;
    if (!ensureTimeValid()) return false;
    Serial.printf("TARS: TTS chars = %u\n", (unsigned)text.length());
    ...
    while (http.connected() && (contentLength > 0 || contentLength == -1)) {
        ...
        if (available > 0) {
            ...
            if (readSize > sizeof(buffer)) readSize = sizeof(buffer);
            int read = stream->readBytes(buffer, readSize);
            if (read > 0) {
                file.write(buffer, read);
                total += read;
                lastData = millis();
                if (contentLength > 0) contentLength -= read;
            }
        } else {
...
// ============================================================
// A2DP CALLBACKS & safe handlers (MODIFIED)
// ============================================================
...
// Original data callback (defensive)
...
// Safe connection-state wrapper that does NOT deref a2dpSource
...
// ============================================================
// START BLUETOOTH (modified to use safe callbacks)
// ============================================================
bool startBluetooth() {
    ...
    if (!btConnected) {
        ...
        printHeap("BT_FAILED");
        // Bluetooth gagal = langsung shutdown dan release object.
        releaseBluetooth();
        return false;
    }
...
bool playMP3() {
    ...
    Serial.println("TARS: PLAY START");
    playbackRunning = true;
    pcmRing.clear();

    if (!startBluetooth()) {
        Serial.println("TARS: BT FAILED - AUDIO ABORT");
        mp3File.close();
    ...
    mp3Decoder.setMaxPCMSize(4096);
    mp3Decoder.setMaxFrameSize(2048);

    Serial.println("HELIX: max PCM = 4096");
    Serial.println("HELIX: max frame = 2048");

    printHeap("BEFORE_HELIX_BEGIN");

    if (!mp3Stream.begin()) {
        Serial.println("TARS: MP3 DECODER START FAILED");
        mp3File.close();
        stopBluetooth();
        playbackRunning = false;
        cleanupAudioSession();
    ...
    Serial.println("TARS: MP3 DECODER READY");
    printHeap("AFTER_HELIX_BEGIN");

    StreamCopy mp3Copier(mp3Stream, mp3File, MP3_COPY_BUFFER);
    mp3Copier.setCheckAvailableForWrite(false);
    mp3Copier.setCheckAvailable(true);

    uint32_t playStart = millis();
    uint32_t lastProgress = millis();
    size_t lastPosition = 0;
    bool decoderFinished = false;
    uint32_t oledAudioWaitStart = millis();

    while (!btAudioStarted && btConnected && millis() - oledAudioWaitStart < 5000) {
        delay(5);
    }
    ...
    while (millis() - playStart < PLAY_TIMEOUT_MS) {
        oledUpdateTyping(true);
        size_t position = mp3File.position();
        size_t availablePCM = pcmRing.available();
        ...
        if (!btConnected) {
            Serial.println("TARS: Bluetooth LOST");
            break;
        }
    ...
    if (pcmRing.available() > 0) {
        Serial.printf("TARS: PCM DRAIN TIMEOUT, remaining=%u\n", (unsigned)pcmRing.available());
    } else {
        Serial.println("TARS: PCM EMPTY");
    }

    // A2DP FINAL TAIL
    Serial.printf("TARS: A2DP TAIL %ums\n", (unsigned)A2DP_TAIL_MS);
    uint32_t tailStart = millis();
    uint32_t lastCallback = btCallbackCalls;
    while (millis() - tailStart < A2DP_TAIL_MS) {
        oledUpdateTyping(true);
        delay(10);
        if (!btConnected) break;
        if (btCallbackCalls != lastCallback) lastCallback = btCallbackCalls;
    }

    Serial.printf("TARS: A2DP callbacks final = %u\n", (unsigned)btCallbackCalls);
    ...
    if (!decoderFinished) {
        Serial.println("TARS: PLAYBACK TIMEOUT");
        oledShowReady();
        return false;
    }

    // Ensure text finished
    while (oledTyping) {
...
void handleQuestion(const String &question) {
    ...
    Serial.println();
    Serial.println("TARS: COMMAND RECEIVED");
    ...
    if (!connectWiFi(true)) {
        Serial.println("TARS: WIFI ERROR");
        oledShowReady();
        return;
    }
    ...
    if (answer.length() == 0) {
        Serial.println("TARS: AI EMPTY");
        oledShowReady();
        return;
    }

    oledStartTyping(answer);

    if (!downloadTTS(answer)) {
        Serial.println("TARS: TTS FAILED");
        oledShowReady();
        return;
    }
    ...
    if (connectWiFi(false)) {
        if (!isTimeValid()) {
            Serial.println("TARS: TIME LOST - NTP REQUIRED");
            syncNTP();
        }
    }
...
void setup() {
    ...
    Serial.println("BT   : QUEUE 8 / STACK 2048");
    Serial.println("PRIME: DISABLED");
    Serial.println("A2DP : NEW OBJECT PER SESSION");
