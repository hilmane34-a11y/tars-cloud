...
void testDAC(){
  ...
  const int rate=22050,freq=1000,samples=256;
  uint16_t buf[samples*2];

  for(int i=0;i<samples;i++){
    float s=sin(2.0f*PI*freq*i/rate);
  ...
  for(int n=0;n<100;n++){
    size_t written=0;
    i2s_write(DAC_PORT,buf,sizeof(buf),&written,portMAX_DELAY);
  }
...
String readHTTPBody(WiFiClientSecure &c){
  ...
  Serial.printf("TARS: STT TRANSFER=%s LENGTH=%d\r\n",
...
public:
  void resetStats(){calls=pcmBytes=outBytes=errors=0;peak=0;}

  void printStats(){
    Serial.printf(
      "TARS: DAC CALL=%lu PCM=%lu OUT=%lu PEAK=%ld ERROR=%lu\r\n",
      (unsigned long)calls,(unsigned long)pcmBytes,
      (unsigned long)outBytes,(long)peak,(unsigned long)errors
    );
  }
  ...
  void setAudioInfo(AudioInfo i)override{
    ...
    Serial.printf("TARS: DAC AUDIO %uHz %ubit %uch\r\n",
  ...
  size_t write(const uint8_t *d,size_t n)override{
    ...
    if(frames>BUF/4)frames=BUF/4;

    for(size_t i=0;i<frames;i++){
      int32_t v;

      if(info.channels==1){
        v=(int16_t)(d[i*2]|((uint16_t)d[i*2+1]<<8));
      }else{
        int16_t l=(int16_t)(d[i*4]|((uint16_t)d[i*4+1]<<8));
        int16_t r=(int16_t)(d[i*4+2]|((uint16_t)d[i*4+3]<<8));
    ...
    size_t written=0;
    esp_err_t e=i2s_write(DAC_PORT,buffer,frames*4,&written,portMAX_DELAY);
    ...
    outBytes+=written;
    return written?frames*info.channels*2:0;
  }
};
...
void setup(){
  ...
  // WiFi configuration/connect + NTP
  wifiManagerBegin();
  ...
  if(wifiOK()){
    ...
    // DAC test dijalankan setelah konfigurasi WiFi selesai
    if(dacOK)testDAC();
  }
