...
String ask(const String&q){
  ...
  JsonDocument x;if(deserializeJson(x,r))return "";
...
bool streamAudio(const String&text){
  ...
  int b0=stream->peek();
  Serial.printf("TARS: AUDIO FIRST BYTE=0x%02X\n",b0<0?0:b0);

  if(b0>=0)
    Serial.printf("TARS: MP3 HEADER FIRST BYTE=%02X\n",(uint8_t)b0);
  ...
  Serial.println("TARS: MP3 COPIER READY");
  ...
  while(true){
    ...
    if(copied){
      ...
      if(!started){
        started=true;
        Serial.printf("TARS: MP3 DECODE START=%lu ms\n",
  ...
  Serial.printf("TARS: MP3 COPY CALLS=%lu\n",(unsigned long)copyCount);
