...
const uint32_t LISTEN_MAX_MS=15000,PREROLL_MS=500;
const int32_t MIC_THRESHOLD=6000,MIC_SILENCE=4500;
const uint32_t OLED_TYPE_MS=5;
...
bool initDAC(){
  ...
  Serial.println("TARS: DAC IDLE MUTE");return true;
}
...
bool recordMic(){
  ...
  while(!voiceStarted && millis()-listenStart<LISTEN_MAX_MS ||
    ...
    // Simpan rolling pre-roll selama belum ada suara
    if(!voiceStarted){
      for(size_t i=0;i<count;i++){
        pre[prePos]=pcm[i];
    ...
    if(!voiceStarted){
      if(blockPeak>=MIC_THRESHOLD){
        if(++activeBlocks>=2){
          ...
          // Tulis pre-roll dari sample tertua -> terbaru
          ...
          // Blok saat suara terdeteksi juga masuk
  ...
  uint32_t recordTime=millis()-listenStart;
...
class DACOut:public AudioStream{
  ...
  void run(){
    ...
    while(active){
      ...
      int32_t v=((int32_t)sample*3)/4;peak=max(peak,abs(v));

      if(!started){dacCenter();next=micros();frac=0;started=true;}

      int32_t out=(v+32768+128)>>8;out=constrain(out,0,255);
