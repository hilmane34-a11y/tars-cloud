...
void oledStartSpeak(const String&s){oledStatus="SPEAKING";oledText=s;oledTypePos=0;oledLastType=millis();}
...
bool bootWiFi(){
  ...
  while(WiFi.status()!=WL_CONNECTED&&millis()-start<30000){wifiManagerConnect(false);delay(100);yield();}
...
void sttEvent(WStype_t type,uint8_t*payload,size_t length){
  ...
  if(t=="ready"){sttReady=true;Serial.println("TARS: STT REALTIME READY");}
    ...
    sttError=true;sttDone=true;Serial.print("TARS: STT ERROR = ");Serial.println(j["error"].as<String>());
  }
}
  ...
  for(;;){
    ...
    if(!voice){
      ...
      if(peak>=MIC_THRESHOLD){
        ...
        if(n&&!sttError)sttWS.sendBIN((uint8_t*)sendBuf,n*2);samples+=preCount;
...
String ask(const String&q){
  ...
  Serial.printf("TARS: ASK HTTP=%d TIME=%lu ms",code,(unsigned long)(millis()-t));Serial.println();
...
bool streamAudio(const String&url,const String&text){
  if(!wifiOK())return false;
  WiFiClientSecure c;c.setInsecure();c.setTimeout(15000);HTTPClient h;uint32_t totalStart=millis();
  ...
  Serial.print("TARS: TTS HEADER STATUS=");Serial.println(engine.length()?engine:"<none>");
  Serial.print("TARS: TTS HEADER FORMAT=");Serial.println(fmt.length()?fmt:"<none>");
  ...
  Serial.print("TARS: X-TARS-TTS=");Serial.println(engine.length()?engine:"<none>");
  Serial.print("TARS: X-TARS-TTS-FORMAT=");Serial.println(fmt.length()?fmt:"<none>");
  Serial.println("TARS: BINARY AUDIO RESPONSE");
  Serial.print("TARS: CONTENT-TYPE FORMAT=");Serial.println(isWav?"WAV":"MP3");
  ...
  if(!isWav){
    ...
    playing=true;
    bool started=false;uint32_t start=millis(),lastData=start;
    Serial.println("TARS: AUDIO STREAM START");
    while(true){
      bool copied=copier.copy(),available=stream->available();
      if(copied){lastData=millis();if(!started){started=true;
  ...
  copier.begin(wavDec,*stream);wavDec.addNotifyAudioChange(stereoOut);wavDec.begin();playing=true;
  bool started=false;uint32_t start=millis(),lastData=start;
  Serial.println("TARS: AUDIO STREAM START");
  while(true){
    bool copied=copier.copy(),available=stream->available();
  ...
  Serial.printf("TARS: AUDIO STREAM=%lu ms\n",(unsigned long)(millis()-start));
  wavDec.end();stereoOut.end();h.end();playing=false;
  Serial.printf("TARS: AUDIO TOTAL=%lu ms\n",(unsigned long)(millis()-totalStart));
  oledSetListening();return started;
}

void processQuestion(const String&q){
  String answer=ask(q);if(!answer.length()){oledSetStatus("ASK ERROR");return;}
  String url=String(TARS_CLOUD_URL)+"/tts";uint32_t t=millis();bool ok=streamAudio(url,answer);
...
void loop(){
  ...
  String q=recordRealtime();if(q.length())processQuestion(q);else oledSetListening();
  delay(1);
}
