...
const uint32_t SILENCE_MS=1000;
const uint32_t PREROLL_MS=700;
const int32_t MIC_THRESHOLD=8000;
const int32_t MIC_SILENCE=6000;
const uint32_t OLED_TYPE_MS=39;
  ...
  for(;;){
    ...
    if(now-oledLastWave>=OLED_WAVE_MS){
      ...
      if(oledText.length()){
        ...
        if(line.length()&&lineCount<20)
          lines[lineCount++]=line;

        const int PER_PAGE=4;
        int page=lineCount?(lineCount-1)/PER_PAGE:0;
...
bool recordMic(){
  ...
  for(;;){
    ...
    /* Menunggu suara tanpa timeout */
    if(!voice){
      for(size_t i=0;i<count;i++){
        preroll[prePos]=pcm[i];
