...
MP3SYM(follow) MP3SYM(mundur) MP3SYM(maju)
MP3SYM(online) MP3SYM(offline) MP3SYM(angkat)
...
bool initDAC(){
 ...
 cfg.sample_rate=22050;cfg.channels=1;cfg.bits_per_sample=16;
...
bool syncTime(){
 ...
 for(int a=1;a<=4;a++){
  Serial.printf("TARS: NTP %d/4\n",a);oledSetStatus("NTP "+String(a)+"/4");
  uint32_t st=millis();
...
void sttEvent(WStype_t type,uint8_t*payload,size_t length){
 ...
 if(t=="ready"){
  ...
  Serial.print("TARS: STT ERROR = ");Serial.println(e);oledSetStatus("STT ERROR");
 }
}
 ...
 for(;;){
  ...
  if(!voice){
   ...
   if(peak>=MIC_THRESHOLD||rms>=1800){
    ...
    samples+=preCount;
    Serial.printf("TARS: VOICE PEAK=%ld RMS=%lu\n",(long)peak,(unsigned long)rms);
   }
  }else{
...
bool recordOfflineWAV(){
 ...
 File f=LittleFS.open("/stt.wav","w");
 if(!f){Serial.println("TARS: OFFLINE WAV OPEN ERROR");return false;}
 ...
 if(!voice||!total){
  LittleFS.remove("/stt.wav");
  Serial.println("TARS: OFFLINE AUDIO TOO LOW");return false;
 }
 ...
 File out=LittleFS.open("/stt2.wav","w");
