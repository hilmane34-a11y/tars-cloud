...
const int32_t MIC_THRESHOLD=6000;
...
bool initDAC(){
  ...
  esp_err_t e=i2s_driver_install(
    DAC_PORT,&c,0,nullptr
  );
...
// DIRECT DAC TEST
// GPIO26 = RIGHT
// I2S DILEPAS SEMENTARA
// =====================================================

void testDAC(){
  Serial.println(
    "TARS: DIRECT DAC GPIO26 RIGHT TEST START"
  );
  ...
  Serial.println(
    "TARS: DIRECT DAC GPIO26 RIGHT TEST DONE"
  );
  ...
  Serial.println(
    dacOK?
      "TARS: I2S DAC RESTORED":
      "TARS: I2S DAC RESTORE FAILED"
  );
}
...
bool playMP3(){
  ...
  // Pastikan I2S DAC tersedia setelah direct test
  if(!dacOK){
    dacOK=initDAC();
...
void setup(){
  ...
  // I2S DAC tetap aktif untuk playback MP3
  dacOK=initDAC();
  ...
  if(wifiOK()){
    ...
    // Tes RIGHT DAC setelah WiFi selesai
    if(dacOK)
      testDAC();
  }
