...
void oledType(){
  ...
  String title=singMode?"SINGING":"SPEAKING";
  oledHeader(title.c_str());
...
bool initMic(){
  ...
  i2s_pin_config_t p={
    I2S_PIN_NO_CHANGE,BCLK,WS,
    I2S_PIN_NO_CHANGE,SD
  };
...
bool recordSTT(){
  ...
  const size_t PN=RATE*PREROLL_MS/1000;
  static int16_t pre[PN];
  ...
  while((!voice&&millis()-listenStart<LISTEN_MAX)||
    ...
    if(!voice){
      ...
      if(blockPeak>=MIC_SIL)
        lastVoice=millis();

      // TUNGGU 1 DETIK SETELAH SUARA TERAKHIR
  ...
  Serial.printf(
    "TARS: RECORDED %u samples\r\n",
    (unsigned)samples
  );

  Serial.printf(
    "TARS: WAV SIZE %u bytes\r\n",
    (unsigned)(samples*2+44)
  );

  Serial.printf(
    "TARS: MIC PEAK RAW %ld\r\n",
    (long)peak
  );

  Serial.printf(
    "TARS: CLIPPED SAMPLES %lu\r\n",
    (unsigned long)clips
  );

  Serial.printf(
    "TARS: MIC PEAK WAV %.0f\r\n",
    peak*MIC_ATTEN
  );
  ...
  Serial.println(
    clips?
    "TARS: INPUT CLIPPING DETECTED -> ATTENUATED":
    "TARS: INPUT NO CLIPPING"
  );
...
String stt(){
  ...
  if(!c.connect(host.c_str(),443)){
    f.close();
    Serial.println("TARS: STT CONNECT FAILED");
    return "";
  }
  ...
  // Buffer upload lebih besar agar transfer lebih cepat
  uint8_t buf[IO_BUF];
  ...
  while(f.available()){
    ...
    if(c.write(buf,n)!=n){
      f.close();
      c.stop();
      Serial.println("TARS: STT UPLOAD FAILED");
      return "";
    }
  }
  ...
  Serial.printf(
    "TARS: STT UPLOAD %lums\r\n",
    (unsigned long)(millis()-start)
  );

  // Tunggu header response
  uint32_t t=millis();
  ...
  if(!c.available()){
    c.stop();
    Serial.println("TARS: STT RESPONSE TIMEOUT");
    return "";
  }
  ...
  // Header HTTP dibuang cepat
  while(c.connected()){
    String l=c.readStringUntil('\n');
    if(l=="\r"||!l.length())break;
  }

  // Baca body tanpa delay 3 detik
  String body;
  ...
  // Jika body datang bertahap, tunggu maksimal 8 detik
  t=millis();
...
bool calledTARS(const String&q){
  ...
  while(*p){
    ...
    while(*p&&
      !isAlphaNumeric((unsigned char)*p))
      p++;
...
String cleanTARS(String q){
  ...
  while(start<s.length()){
    ...
    while(
      start<s.length()&&
      !isAlphaNumeric((unsigned char)s[start])
    )start++;
    ...
    while(
      ...
      isAlphaNumeric((unsigned char)s[end])
    )end++;
...
void answer(const String&q){
  ...
  // SEMUA PERINTAH LAIN WAJIB ADA TARS
  if(!calledTARS(q)){
...
void setup(){
  ...
  Serial.println(
    "TARS: DIRECT STT MODE"
  );

  Serial.println(
    "TARS: INMP441 RIGHT CHANNEL"
  );

  Serial.println(
    "TARS: MIC SCK=18 WS=19 SD=34"
  );

  Serial.println(
    "TARS: MIC ATTENUATION = 50%"
  );

  Serial.println(
    "TARS: STT LANGUAGE = INDONESIAN"
  );
  ...
  Serial.println(
    "TARS: OTHER SPEECH = BLOCKED"
  );

  Serial.println(
    "TARS: POST-SPEECH WAIT = 1000ms"
  );
