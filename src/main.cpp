...
/* Hitung posisi akhir halaman = 3 baris */
...
String stt(){
  ...
  /* WAV langsung dari LittleFS -> HTTPS, tanpa copy seluruh file ke RAM */
  while(f.available()){
  ...
  while(!c.available()&&c.connected()&&millis()-waitStart<20000){
    delay(2);
    yield();
  }
