...
float wakeScore(const uint8_t*e,uint16_t s,uint16_t len){
  uint8_t lo=255,hi=0;
  for(uint16_t i=0;i<len;i++){lo=min(lo,e[s+i]);hi=max(hi,e[s+i]);}
  if(hi<=lo+4)return 1;
  float d=0;
  for(int j=0;j<64;j++){
    float q=(float)j*(len-1)/63,i=q;
    uint16_t j0=i,j1=j0+1<len?j0+1:j0;
    float v=e[s+j0]+(e[s+j1]-e[s+j0])*(i-j0);
    v=constrain((v-lo)*255/(hi-lo),0.0f,255.0f);
    d+=fabsf(v-TARS_TEMPLATE[j])/255;
  }
  return d/64;
}
...
bool detectTars(){
  ...
  for(uint16_t l=40;l<=min((uint16_t)90,n);l+=5)
    for(uint16_t s=0;s+l<=n;s++)
      best=min(best,wakeScore(e,s,l));
...
bool recordMic(){
  ...
  while((!voice&&millis()-start<LISTEN_MAX)||(voice&&millis()-vs<REC_MAX)){
    ...
    if(!voice){
      for(size_t i=0;i<c;i++){
        pre[pp]=pcm[i];pp=(pp+1)%PRE;if(pc<PRE)pc++;
      }
      if(bp>=MIC_TH&&++act>=2){
        voice=true;vs=millis();last=vs;
        size_t st=pc==PRE?pp:0;
        for(size_t i=0;i<pc;i++){
          size_t k=(st+i)%PRE;f.write((uint8_t*)&pre[k],2);
        }
...
String body(WiFiClientSecure&c){
  ...
  if(chunk){
    while(c.connected()){
      ...
      int n=strtol(l.c_str(),0,16);if(n<=0)break;
      while(n>0){
        uint8_t z[BUF];size_t w=min((int)sizeof(z),n);
        size_t r=c.readBytes(z,w);if(!r)break;
        b.concat((char*)z,r);n-=r;
      }
      c.readStringUntil('\n');
    }
  }else if(len>=0){
    while((int)b.length()<len&&c.connected()){
      uint8_t z[BUF];int rl=len-b.length();
      size_t w=min((int)sizeof(z),rl),r=c.readBytes(z,w);
      if(!r)break;b.concat((char*)z,r);
    }
  }else b=c.readString();
  return b;
}
...
String stt(){
  ...
  int p=host.indexOf("://");if(p>=0)host=host.substring(p+3);
...
String offlineAnswer(String q){
  ...
  if(q.indexOf("jam berapa")>=0||q.indexOf("tanggal berapa")>=0){
    ...
    const char*m[]={"Januari","Februari","Maret","April","Mei","Juni",
...
const char* offlineFile(const String&q){
  ...
  if(x.indexOf("hidup jokowi")>=0||x.indexOf("jokowi hidup")>=0)
    return OFFLINE_JOKOWI;

  if(x.indexOf("ceritakan pengalaman")>=0||x.indexOf("pengalaman kamu")>=0)
    return OFFLINE_EXP;
...
bool playOffline(const char*src,const String&text){
  ...
  a.close();b.close();
  oledText=text;oledPos=oledPage=0;singMode=false;
  return playMP3();
}
...
bool downloadMP3(const String&url,const String&text){
  ...
  uint8_t z[BUF];int len=h.getSize();
...
public:
  ...
  void stop(){
    active=false;uint32_t z=millis();
    while(task&&millis()-z<2000)vTaskDelay(1);
    dacMute();
  }
...
void loop(){
  ...
  if(recordMic()){
    ...
    Serial.println("TARS: WAKE ACCEPTED");
    String q=stt();
    LittleFS.remove(STT_FILE);
