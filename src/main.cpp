...
void oledType(){
  ...
  while(i<oledPos){
    char c=oledText[i++];
    if(c=='\n'||x>121){x=3;y+=8;if(c=='\n')continue;}
    if(y>59){oledPage=i-1;i=oledPage;x=3;y=27;oledHeader(title.c_str());}
  ...
  while(oledPos<oledText.length()&&n<5){
    ...
    if(c=='\n'||x>121){x=3;y+=8;if(c=='\n')continue;}
    if(y>59){oledPage=oledPos-1;x=3;y=27;oledHeader(title.c_str());continue;}
    oled.setCursor(x,y);oled.write(c);x=oled.getCursorX();n++;
  }
  oled.display();
}

/* MIC INMP441: SCK18 / WS19 / SD34 / RIGHT */
bool initMic(){
  i2s_config_t c={};
  ...
  c.dma_buf_count=2;c.dma_buf_len=256;c.use_apll=false;
...
void wavHeader(File&f,uint32_t bytes){
  uint8_t h[44]={};
  memcpy(h,"RIFF",4);put32(h+4,bytes+36);
...
bool recordSTT(){
  ...
  while((!voice&&millis()-listenStart<LISTEN_MAX)||
    ...
    if(!voice){
      ...
      if(blockPeak>=MIC_SIL)lastVoice=millis();
...
public:
  ...
  void start(){
    ...
    if(!task)xTaskCreatePinnedToCore(
...
String cleanTARS(String q){
  ...
  int p=s.indexOf("tars");

  if(p>=0)q.remove(p,4);

  q.trim();

  while(q.length()&&strchr(" ,.!?:;",q[0]))
    q.remove(0,1);

  while(q.length()&&strchr(" ,.!?:;",q[q.length()-1]))
    q.remove(q.length()-1);
...
void answer(const String&q){
  ...
  /* PRIORITAS OFFLINE HIDUP JOKOWI */
  ...
  if(singMode){
    ...
    if(downloadMP3(
      String(TARS_CLOUD_URL)+"/sing",clean
    ))
      playMP3();
    else
...
void setup(){
  ...
  Serial.println("TARS: MIC I2S FORMAT = I2S");
