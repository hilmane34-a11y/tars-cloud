...
class AudioRingStream:public Stream{
  ...
  volatile size_t minCount=AUDIO_RING_SIZE,maxCount=0;
  volatile uint32_t underruns=0,readCalls=0,zeroReads=0;
  volatile uint32_t rxChunks=0,rxBytes=0,rxMaxGap=0;
    ...
    while(!abortRx){
      ...
      if(av>0){
        ...
        if(n>0){
          uint32_t now=millis();
          if(lastRx) {
            uint32_t gap=now-lastRx;
            if(gap>rxMaxGap)rxMaxGap=gap;
          }
          lastRx=now;
          push(tmp,(size_t)n);
          received+=n;
  ...
  size_t push(const uint8_t*p,size_t n){
    ...
    size_t c=(size_t)count,freeSpace=AUDIO_RING_SIZE-c;
    if(n>freeSpace)n=freeSpace;

    size_t h=(size_t)head,remain=AUDIO_RING_SIZE-h;
...
public:
  ...
  void printStats(){
    Serial.printf(
      "TARS: AUDIO DIAG RING MIN=%u MAX=%u UNDERRUN=%lu ZERO_READ=%lu READ=%lu\n",
      (unsigned)minCount,(unsigned)maxCount,
      (unsigned long)underruns,(unsigned long)zeroReads,
      (unsigned long)readCalls
    );
    Serial.printf(
      "TARS: AUDIO DIAG RX CHUNKS=%lu BYTES=%lu MAX_GAP=%lu ms\n",
      (unsigned long)rxChunks,(unsigned long)rxBytes,
      (unsigned long)rxMaxGap
    );
  }
  ...
  int read(uint8_t*p,size_t n){
    ...
    size_t c=(size_t)count,take=n<c?n:c;

    if(!take&&running&&!done)
      underruns++;

    if(take){
      size_t t=(size_t)tail,remain=AUDIO_RING_SIZE-t;
...
void oledTask(void*){
  for(;;){
    ...
    if(now-oledLastWave>=OLED_WAVE_MS){
      ...
      oled.setTextColor(SSD1306_WHITE);
      oled.setTextSize(1);
      oled.setCursor(3,0);oled.print("T A R S");
...
bool streamAudio(const String&url,const String&text){
  ...
  if(!isWav){
    ...
    uint32_t copyMaxUs=0,copyZero=0,copySlow=0;
    ...
    while(true){
      ...
      if(copyUs>100000){
        copySlow++;
        Serial.printf(
          "TARS: MP3 COPY SLOW=%lu us RING=%d\n",
          (unsigned long)copyUs,audioRing.available()
        );
      }
    ...
    Serial.printf(
      "TARS: MP3 COPY MAX=%lu us COPY_ZERO=%lu SLOW=%lu\n",
      (unsigned long)copyMaxUs,
      (unsigned long)copyZero,
      (unsigned long)copySlow
    );

    audioRing.printStats();
  ...
  while(true){
    ...
    if(copyUs>100000){
      copySlow++;
      Serial.printf(
        "TARS: WAV COPY SLOW=%lu us RING=%d\n",
        (unsigned long)copyUs,audioRing.available()
      );
    }
  ...
  Serial.printf(
    "TARS: WAV COPY MAX=%lu us COPY_ZERO=%lu SLOW=%lu\n",
    (unsigned long)copyMaxUs,
    (unsigned long)copyZero,
    (unsigned long)copySlow
  );
