...
bool recordSTT(){
  ...
  uint8_t z44[44]={0};f.write(z44,44);
  int16_t pcm[IO_BUF/4];const size_t PN=sizeof(pre)/sizeof(pre[0]);
  ...
  p32(h+28,RATE*2);p16(h+32,2);p16(h+34,16);memcpy(h+36,"data",4);p32(h+40,samples*2);
...
String stt(){
  ...
  String host=TARS_CLOUD_URL;int x=host.indexOf("://");
  if(x>=0)host=host.substring(x+3);
  x=host.indexOf('/');if(x>=0)host=host.substring(0,x);
  ...
  String q=j["text"]|String("");q.trim();
  return q=="null"||q=="NULL"?String(""):q;
}
...
String ask(const String&q){
  ...
  String a=x["response"]|String("");if(!a.length())a=x["answer"]|String("");
  return a;
}
...
bool downloadMP3(const String&url,const String&text){
  ...
  WiFiClient*s=h.getStreamPtr();uint8_t z[IO_BUF];
...
String cleanTARS(String q){
  ...
  q.trim();while(q.length()&&strchr(" ,.!?",q[0]))q.remove(0,1);
  q.trim();return q;
}
...
String localTimeText(){
  ...
  snprintf(s,sizeof(s),"Pukul %02d lewat %02d menit WIB. %s, %02d %s tahun %04d.",
           t.tm_hour,t.tm_min,t.tm_wday>=0?d[t.tm_wday]:"",t.tm_mday,m[t.tm_mon],t.tm_year+1900);
  return String(s);
}
...
void answer(const String&q){
  ...
  if(!calledTARS(q)){
    ...
    oledBase("UNKNOWN","BLOCKED");delay(500);oledBase("STANDBY","LISTENING...");
    return;
  }
...
void setup(){
  ...
  /* NTP hanya sekali setelah WiFi pertama kali berhasil */
  while(!syncTime()){
...
void loop(){
  ...
  if(!q.length()){
    ...
    oledBase("STANDBY","NO TEXT");delay(300);return;
  }
