...
void oledType(){
  ...
  while(i<oledPos){
    ...
    if(y>59){
      oledPage=i-1;i=oledPage;x=3;y=27;
      oledHeader(singMode?"SINGING":"SPEAKING");
    }
    if(y<=59){
...
bool readMic(int16_t*p,size_t maxN,size_t&n,int32_t&peak){
  ...
  for(size_t i=0;i<n;i++){
    /*
      INMP441 mengirim 24-bit dalam slot 32-bit.
      Ambil 24-bit terlebih dahulu lalu skalakan ke PCM16.
    */
    int32_t v=raw[i]>>8;
    int16_t s=(int16_t)constrain(v>>8,-32768,32767);
...
bool recordSTT(){
  ...
  while((!voice&&millis()-start<LISTEN_MAX)||
    ...
    if(!voice){
      ...
      if(millis()-vs>=REC_MIN&&millis()-last>=SILENCE)
        break;
    }
