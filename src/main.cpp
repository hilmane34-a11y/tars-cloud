...
void makeMFCCFrame(const int16_t*s,float*out){
  for(int i=0;i<FFT_N;i++){
    float x=(float)s[i]/32768.0f;
    float p=i?(float)s[i-1]/32768.0f:0;
  ...
  for(int k=0;k<=FFT_N/2;k++){
    ...
    for(int b=0;b<MEL;b++){
      float l=lo+(hi-lo)*b/(MEL+1);
      float c=lo+(hi-lo)*(b+1)/(MEL+1);
      float r=lo+(hi-lo)*(b+2)/(MEL+1);
      float w=0;
...
void normalizeFeatures(){
  for(int c=0;c<MFCC;c++){
    ...
    for(int f=0;f<KWS_FRAMES;f++)
      kwsFeat[f][c]=constrain((kwsFeat[f][c]-mean)/sd,-3.0f,3.0f)*127.0f/3.0f;
  }
}
...
Command classify(){
  ...
  if(d[best]>DTW_TH||d[second]-d[best]<DTW_MARGIN)
    return UNKNOWN;
...
public:
  ...
  void start(){
    ...
    if(!task)
      xTaskCreatePinnedToCore(taskFn,"TARS_DAC",2048,this,2,&task,1);
  }
...
void setup(){
  ...
  Serial.println("TARS: DTW LOW DRAM KWS");
