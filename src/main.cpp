...
const uint32_t STREAM_EOF_IDLE_MS=1000;
...
public:
  ...
  void report(){
    ...
    uint64_t df=decBytes/fb;
    uint64_t af=dacBytes/fb;
    ...
    if(now-oledLastWave>=OLED_WAVE_MS){
      ...
      if(oledText.length()){
        String s=oledText.substring(
          0,
          min(oledTypePos,(uint32_t)oledText.length())
        );
