...
bool recordMic(){
  ...
  Serial.printf("TARS: MIC READ=%lu ERROR=%lu SAMPLES=%lu\r\n",
                (unsigned long)reads,(unsigned long)errors,
...
class DACOut:public AudioStream{
  ...
  uint32_t calls=0,pcmBytes=0,dacSamples=0;
  uint32_t errors=0,sampleRate=PLAY_RATE;
  int32_t peak=0;
...
public:
  ...
  void stopDAC(){
    ...
    while(task){
      ...
      if(millis()-t>2000){
        Serial.println("TARS: DAC TASK TIMEOUT");
        break;
      }
    }
